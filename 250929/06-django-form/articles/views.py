...
#     # 1. 사용자 입력 데이터를 통째로 Form 클래스에 인자로 넣어서 인스턴스를 생성
...
def create(request):
    ...
    # 요청 메서드가 POST가 아니라면?
    # 요청 메서드가 GET이라면?
    # 우리가 GET/POST만 배웠는데 PUT, DELETE ..등 좀 많다. -> 고려할 게 많다.
    # POST 가 else로 빠지면 오류가 날 확률이 높아진다.
    # 나머지는 중요한 로직이 아니어서 상관 없다.
    # GET이 아니라면 이라는 로직으로 POST를 처리하기엔 POST가 너무 중요한 로직이다.
    else:
        form = ArticleForm()
...
#     # instance가 없으면 생성, instance가 있으면 갱신
